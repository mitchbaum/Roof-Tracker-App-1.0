...
//            let alertController = UIAlertController(title: "Empty Check Date", message: "You have not entered a date for the check", preferredStyle: .alert)
...
//        }
        ...
        guard let checkDate = dateFormatter.date(from: checkText)
            ...
            let alertController = UIAlertController(title: "Invalid Date", message: "Double check the format. MM/dd/yyyy", preferredStyle: .alert)
            alertController.addAction(UIAlertAction(title: "Ok", style: .default, handler: nil))
            present(alertController, animated: true, completion: nil)
            return
        }
