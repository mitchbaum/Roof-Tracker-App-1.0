...
//    }
    ...
    private func fetchItems() {
        ...
        var insChecksRecieved = fileItems.filter { (item) -> Bool in
            return item.type == "Insurance"
        }
        // filter staff for "Executives"
        var insChecksRecievedAndPaid = fileItems.filter { (item) -> Bool in
            return item.type == "Insurance PAID"
        }
        // filter staff for "Executives"
        var personalPaymentMade = fileItems.filter { (item) -> Bool in
            return item.type == "Personal"
        }
        // filter for insurance and paid
        // filter senior management for "Executives"
        var RCVworkToDo = fileItems.filter { (item) -> Bool in
            return item.type == "ACV owed to HO"
        }
        // filter staff for "Executives"
        var ACVtoHO = fileItems.filter { (item) -> Bool in
            return item.type == "RCV work to do"
        }
        // filter staff for "Executives"
        var cashToDo = fileItems.filter { (item) -> Bool in
            return item.type == "Cash work to do"
        }
        // sort the rows in each section by time added to itemsController
        insChecksRecieved.sort(by: {$0.timeStamp ?? Date() > $1.timeStamp ?? Date()})
        insChecksRecievedAndPaid.sort(by: {$0.timeStamp ?? Date() > $1.timeStamp ?? Date()})
        personalPaymentMade.sort(by: {$0.timeStamp ?? Date() > $1.timeStamp ?? Date()})
        RCVworkToDo.sort(by: {$0.timeStamp ?? Date() > $1.timeStamp ?? Date()})
        ACVtoHO.sort(by: {$0.timeStamp ?? Date() > $1.timeStamp ?? Date()})
        cashToDo.sort(by: {$0.timeStamp ?? Date() > $1.timeStamp ?? Date()})
...
//
    ...
    private func editHandlerFunction(action: UITableViewRowAction, indexPath: IndexPath) {
        ...
        if item.type == "Insurance" || item.type == "Insurance PAID" || item.type == "Personal" {
            ...
            editCheckController.itemInformation = item.itemInformation
            editCheckController.fileItem = item
...
//            print("checkAmount = ", item.itemInformation)
            ...
            editLineItemController.itemInformation = item.itemInformation
            editLineItemController.fileItem = item
...
//        overviewLabel.heightAnchor.constraint(equalToConstant: 40).isActive = true
        ...
        if file?.cocSwitch == true {
            header.addSubview(finalCOCTotalLabel)
            finalCOCTotalLabel.leftAnchor.constraint(equalTo: headerView.leftAnchor, constant: 16).isActive = true
            finalCOCTotalLabel.topAnchor.constraint(equalTo: summaryButton.bottomAnchor).isActive = true
        ...
        header.addSubview(outOfPocketLabel)
        outOfPocketLabel.leftAnchor.constraint(equalTo: headerView.leftAnchor, constant: 16).isActive = true
        ...
        header.addSubview(outOfPocketLabelInfo)
        outOfPocketLabelInfo.rightAnchor.constraint(equalTo: header.rightAnchor, constant: -16).isActive = true
